...
You do not need to attach at all for routine supervision: from an active firstmate session with `FM_HOME` exported, the first mate reads crew windows itself with `bin/fm-peek.sh fm-<id>` (a bounded, read-only capture) and steers a crew with `bin/fm-send.sh fm-<id> "<text>"` when it needs to intervene.
