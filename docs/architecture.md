...
A status log's own "done: ... checks green" line is trusted during an active run only if it can plausibly belong to that run - it names the run's id, the run already has a ci step, or a ci step status is resolvable; otherwise it is a leftover from an earlier run (e.g. a rebase restarted validation) and is flagged superseded so a fresh early run is not short-circuited to done.
...
`fm-send.sh` selects a pre-Enter popup-settle for slash commands and for codex `$...` skill invocations using the target's recorded `harness=` meta, then adds its own `FM_SEND_SETTLE` pause after successful text sends so immediate peeks catch the receiving turn starting; the sub-supervisor uses only the shared submit core and does not pay that post-submit pause. Those same popup-risk sends also require a verified submit: an unreadable ("unknown") composer verdict, normally treated as sent, is instead a hard failure for slash commands and codex `$...` invocations - UNLESS the backend's busy-state shows an idle-to-busy transition caused by this particular send. `fm-send.sh` snapshots busy-state before sending and again after; only a pane that was not busy before and is busy after counts as proof this send is what woke the agent, so a pane that was already busy beforehand still hard-fails on an unknown verdict. Plain text sends stay lenient on "unknown" regardless.
...
`fm-watch.sh` polls each window's backend for a busy state: tmux's dispatcher runs its own busy-footer regex over the pane's tail and reports `busy`/`unknown` directly (reusing the same bounded tail the watcher already captured for hashing, so no extra backend call), while zellij, orca, and cmux have no native primitive at all and always report unknown, preserving the original pane-tail-regex fallback in the watcher itself; herdr's `agent.get` semantic state (working/idle/done/blocked) is consulted first for stale detection, with unknown native states falling back to the same regex.
...
When called with `FM_HOME=<this-firstmate-home>` or when `FM_HOME` is already set to the active firstmate home, bare `fm-send.sh fm-<id>` requests to a live `kind=secondmate` are prefixed with the from-firstmate marker from `bin/fm-marker-lib.sh`, so the secondmate returns terse answers through status lines and detailed answers through docs plus status pointers instead of replying only in its own chat.
...
Before merging it also asserts the PR's check rollup is green: no checks configured is allowed, but pending, failing, canceled, or unknown states refuse the merge with no override flag.
