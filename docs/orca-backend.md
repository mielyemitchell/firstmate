...
You do not need to open the app for routine supervision: from an active firstmate session with `FM_HOME` exported, `bin/fm-peek.sh <id>` reads a task's terminal without opening Orca, and `bin/fm-send.sh <id> "<text>"` steers it (the stable `fm-<id>` alias also works; Enter and Ctrl-C are supported; Escape is not).
