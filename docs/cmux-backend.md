...
| Busy state                      | _(no native primitive)_                                                                                                                                                        | cmux has agent-awareness elsewhere (Claude Code hooks integration, session-resume tokens) but exposes nothing over the socket API for generic busy/idle classification; `surface.health`/`surface-health` is render health, not agent status. `fm_backend_busy_state`'s dispatcher (`bin/fm-backend.sh`) falls through to `unknown` for cmux via its wildcard case, like zellij/Orca - tmux differs now, since it has its own case arm running a positive busy-footer regex over the pane tail, but cmux still has no positive signal at all, so the watcher's existing pane-hash + regex path is the only busy-state source for this backend. |
