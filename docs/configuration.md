...
`bin/fm-reconcile-stale.sh` reuses that same tracked/live matching (`fm-fleet-map-lib.sh`) plus a conservative landed-work assessor (`fm-landed-work-lib.sh`). Its `kind=secondmate` (always blocked, points at `fm-teardown.sh`) and `kind=scout` (gated on the report's existence alone) carve-outs mirror `fm-teardown.sh`, but the underlying content check is deliberately more conservative than `fm-teardown.sh` and omits its PR-head patch-id containment fallback, so a squash merge that `fm-teardown.sh` recognizes as landed only through that fallback reports unlanded/blocked here instead - use `bin/fm-teardown.sh` directly for those cases.
