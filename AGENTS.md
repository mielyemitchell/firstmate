...
`bin/fm-reconcile-stale.sh` reuses that same matching to reconcile it: its default mode is a dry run that reports every stale-tracked record together with a landed-work assessment (reusing `fm-teardown.sh`'s own landed-work semantics, including the secondmate-always-blocked and scout-report-gates-clean carve-outs) and any operator-untracked agents, and it writes nothing.
