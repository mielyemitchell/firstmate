...
"Routine" means reversible by a follow-up PR, inside the task's stated scope, no schema or data migration, no auth or permission surface, no secret handling, no deletion of user data, and CI green; a finding failing any clause escalates even under yolo.
...
Use `bin/fm-tasks-axi.sh` for firstmate's routine backlog mutations when the configured backend selects compatible `tasks-axi`, and use the documented manual path otherwise; keep only the configured recent Done entries.
