...
- `TASKS_AXI: repo-root data/backlog.md differs from FM_HOME backlog - use bin/fm-tasks-axi.sh so tasks-axi runs from <home>` - the default backlog backend is selected and bootstrap found evidence that a bare `tasks-axi` command may have written the repo-root backlog instead of the active home.
  Inspect both backlog files, reconcile the intended content, and keep using the wrapper for routine backlog mutations.
