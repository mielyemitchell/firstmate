...
run_matrix_entry() {
  ...
  case "$entry" in
    codex)
      ...
      printf '%s' "$payload" | env -u FM_HOME "$CHECK" >"$out_file" 2>"$err_file"
      rc=$?
      ;;
    claude)
      payload=$(jq -cn --arg command "$cmd" '{tool_name:"Bash",tool_input:{command:$command}}')
      printf '%s' "$payload" | env -u FM_HOME "$CHECK" --claude >"$out_file" 2>"$err_file"
      rc=$?
      ;;
    grok)
      ...
      ;;
    opencode|pi)
      env -u FM_HOME "$CHECK" --command "$cmd" >"$out_file" 2>"$err_file"
      rc=$?
      ;;
    *)
