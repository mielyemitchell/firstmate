...
test_tasks_axi_cwd_trap_warning() {
  ...
  expected=$(printf 'TASKS_AXI: repo-root data/backlog.md differs from FM_HOME backlog - use bin/fm-tasks-axi.sh so tasks-axi runs from %s' "$(cd "$home" && pwd -P)")
  [ "$out" = "$expected" ] || fail "diverged FM_HOME backlog should warn while routine availability stays silent"$'\n'"expected: $expected"$'\n'"got: $out"
