...
fm_tasks_axi_compatible || { echo "skip: tasks-axi lacks required compatibility"; exit 0; }
tasks-axi hold --help 2>/dev/null | grep -F -- '--kind captain' >/dev/null \
  || { echo "skip: tasks-axi lacks captain-hold contract"; exit 0; }
