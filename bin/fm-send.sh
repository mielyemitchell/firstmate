...
# Usage: fm-send.sh [--expect-ack <minutes>] <target> [--resolve-key <key>]... [--fire-and-forget <delivery-id>] <text...>
...
if [ "${1:-}" = --expect-ack ]; then
  [ "$#" -ge 3 ] || { echo "error: --expect-ack requires a positive integer minute count and a target" >&2; exit 1; }
  EXPECT_ACK_MINUTES=${2:-}
  shift 2
  case "$EXPECT_ACK_MINUTES" in
    ''|*[!0-9]*|0) echo "error: --expect-ack requires a positive integer minute count" >&2; exit 1 ;;
  esac
fi
...
if [ -n "$EXPECT_ACK_MINUTES" ] && [ -z "$TARGET_SELECTOR" ]; then
...
if [ "${1:-}" = "--key" ]; then
  ...
  if [ -n "$EXPECT_ACK_MINUTES" ]; then
    ACK_TARGET_ID=$(fm_send_id_from_meta "$TARGET_META")
    ACK_SENT_AT=$(date +%s)
  ...
  if [ "$INBOX_PLANE" = 1 ] && [ "$TARGET_BACKEND" = remote ]; then
    ...
    if [ -n "$EXPECT_ACK_MINUTES" ]; then
      fm_ack_record "$STATE" "$ACK_TARGET_ID" "$ACK_SENT_AT" "$ACK_DEADLINE" "$ACK_PRE_SIG" "$ACK_PRE_LINES" "$RESOLVE_ANSWER_TEXT" || true
    fi
    exit 0
  fi
