...
# (an agent mid-turn). Scans a 40-line tail like fm-watch.sh. Optional <tail40>
# reuses an already-captured tail instead of issuing a fresh capture-pane call.
fm_pane_is_busy() {  # <target> [tail40]
  local win=$1 tail40=${2:-}
  if [ -z "$tail40" ]; then
...
#   - fm-send normally fails only on "pending" (lenient: a positively-confirmed
#     swallow), so an unreadable pane never turns a normal steer into a false
#     error — except for its own popup-risk sends (slash commands, and codex
#     `$...` invocations), which fm-send.sh additionally fails on "unknown".
