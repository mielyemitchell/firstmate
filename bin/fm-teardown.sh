...
best_effort_backend_kill() {  # <backend> <target> <zellij-tab-id> <label>
  set +e
  fm_backend_kill "$@" 2>/dev/null
  set -e
  return 0
}
  ...
  if fm_tasks_axi_backend_available "$CONFIG"; then
    case "$KIND" in
      scout)
        ...
        done_cmd="bin/fm-tasks-axi.sh done $ID --report $report_path"
        ;;
      *)
        if [ "$MODE" = local-only ]; then
          done_cmd="bin/fm-tasks-axi.sh done $ID --note \"local main\""
        else
          pr=$PR_URL
          if [ -n "$pr" ]; then
            done_cmd="bin/fm-tasks-axi.sh done $ID --pr $pr"
          else
            done_cmd="bin/fm-tasks-axi.sh done $ID --pr PR_URL"
          fi
        fi
        ;;
    esac
    printf '%s\n' "Backlog: $ID just finished. Run $done_cmd, then run bin/fm-tasks-axi.sh ready for dependency-cleared candidates, check date gates, and dispatch only work whose blockers are gone and date is due."
  else
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ -n "$child_t" ]; then
      if [ "$child_backend" = zellij ]; then
        ...
        ( unset FM_ROOT_OVERRIDE; FM_HOME=$home FM_ROOT=$home best_effort_backend_kill "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "fm-$child_id" )
      else
        best_effort_backend_kill "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "fm-$child_id"
      fi
    fi
    if [ "$child_kind" = secondmate ]; then
      child_home=$(meta_value "$child_meta" home)
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  if [ -n "$T_ORCA" ]; then
    best_effort_backend_kill "$BACKEND" "$T" "$(meta_value "$META" zellij_tab_id)" "fm-$ID"
  fi
...
fm_ack_prune_task "$STATE" "$ID" || true
