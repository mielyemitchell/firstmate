...
7. Update roadmap state only as required by the execution artifact after the local batch state changes."
    ...
    ;;
  *)
    CAMPAIGN_BRANCH_SETUP="2. Do not attempt to check out the default branch. The pooled clone holds it, so this worktree starts detached by design. Create the first slice or batch branch directly from the launch commit with \`git switch -c fm/$ID/<batch-slug>\`; when the execution skill preflight expects a named default branch, the current detached base commit stands in for it. Name EVERY campaign branch under the \`fm/$ID/\` prefix - firstmate's teardown protection and ref cleanup track that prefix, and a branch outside it can strand work."
...
8. After firstmate reports a batch PR merged, start the next batch branch from the updated default branch, not the launch commit: \`git fetch origin\` then \`git switch -c fm/$ID/<next-batch-slug> origin/<default-branch>\`, so each batch PR contains only its own batch's diff."
...
# Setup
You are in a persistent, isolated git worktree of $REPO, at a detached HEAD on a clean default-branch commit. It stays yours for the whole roadmap; firstmate tears it down only after the campaign closes.
