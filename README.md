...
- **Incident guardrails** - `fm-freeze.sh on` parks the whole fleet until you lift it; `fm-fleet-map.sh` diagnoses tracked state against live endpoints; `fm-reconcile-stale.sh` cleans only provably dead, landed task state; and `fm-usage-tripwire.sh` alarms on abnormal token or session usage bursts.
